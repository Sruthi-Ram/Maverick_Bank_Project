...
public class SecurityConfig {
	...
	@Autowired
	private JWTAuthenticationFilter jwtAuthFilter; // Inject your JWT filter here
	@Autowired
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		http.csrf(csrf -> csrf.disable()).cors(cors -> cors.configurationSource(corsConfigurationSource())) // Enable
				...
				.authorizeHttpRequests(authorizeRequests -> authorizeRequests.requestMatchers("/",
						"/api/v1/users/login", "/api/v1/users/register", "/swagger-ui/**", "/v3/api-docs/**")
						.permitAll().anyRequest().authenticated())
				.sessionManagement(session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS))
				.httpBasic(Customizer.withDefaults()) // Keep this if you need basic auth for other parts
